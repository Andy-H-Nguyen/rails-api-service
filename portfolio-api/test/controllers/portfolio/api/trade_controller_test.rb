  ...
  class TradeControllerTest < ActionDispatch::IntegrationTest
    ...
    test "trade#create" do
      assert_difference -> { Trade.count } => 1 do
        post addTrade_path, params: { trade: { stock_ticker: "ABC", price: 7.77, quantity: 777, order_type: 'buy'} }, as: :json
      end
      ...
      assert_equal(777, response.parsed_body["data"]["quantity"])    
      assert_equal('buy', response.parsed_body["data"]["order_type"])
    end

    test "trade#update" do
      assert_no_changes -> { Trade.count } do
        post "/portfolio-api/updateTrade/#{portfolio_api_trades(:google).id}", params: { trade: { stock_ticker: "RELI", price: 7.89, quantity: 789, order_type: 'sell'} }, as: :json
      end
      ...
      assert_equal(789, response.parsed_body["data"]["quantity"]) 
